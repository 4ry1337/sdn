...
import { FloodlightDesc } from './types'

export const floodlight_fetch_switches_desc = async ( url: string, dpid: string ): Promise<FloodlightDesc> => {
  try {
    let desc_response = await axios.get( `${url}/wm/core/switch/${dpid}/desc/json`, { timeout: 5000 } )
    let { desc } = z.object( { desc: FloodlightDescSchema } ).parse( desc_response.data )
    return desc
  } catch ( error ) {
