...
import { FloodlightSummery } from './types'

export async function floodlight_fetch_controller_summary( url: string ): Promise<FloodlightSummery> {
  try {
    const response = await axios.get( `${url}/wm/core/controller/summary/json`, {
      timeout: 5000,
      headers: { 'Accept': 'application/json' },
    } )
    return FloodlightSummerySchema.parse( response.data )
  } catch ( error ) {
    if ( error instanceof z.ZodError ) {
      throw new Error( `Invalid controller summary data from Floodlight: ${error.message}` )
    }
