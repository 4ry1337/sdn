...
import { FloodlightUptime } from './types'
import { FloodlightUptimeSchema } from './schema'

export async function floodlight_fetch_controller_uptime( url: string ): Promise<FloodlightUptime> {
  try {
    const response = await axios.get( `${url}/wm/core/system/uptime/json`, {
      timeout: 5000,
      headers: { 'Accept': 'application/json' },
    } )
    return FloodlightUptimeSchema.parse( response.data )
  } catch ( error ) {
    if ( error instanceof z.ZodError ) {
      throw new Error( `Invalid controller uptime data from Floodlight: ${error.message}` )
    }
