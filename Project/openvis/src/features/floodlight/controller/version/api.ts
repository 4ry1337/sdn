...
import { FloodlightVersion } from './types'

export async function floodlight_fetch_controller_version( url: string ): Promise<FloodlightVersion> {
  try {
    const response = await axios.get( `${url}/wm/core/version/json`, {
      timeout: 5000,
      headers: { 'Accept': 'application/json' },
    } )
    return FloodlightVersionSchema.parse( response.data )
  } catch ( error ) {
    if ( error instanceof z.ZodError ) {
      throw new Error( `Invalid controller version data from Floodlight: ${error.message}` )
    }
